package ioserver.mapbuilderIO;

import ioserver.PortalIO;
import ioserver.PositionIO;
 ...
 * Rechtecke werden durch ihre Rechteckige Form und ihre schwarze Farbe erkannt. Während Portale durch die Farbe Rot(255,0,0)
 * erkannt werden.
 */
public class MapBuilder {
     ...
     * @return Diese Methode gibt eine Liste aller schwarzen Rechtecke, die in dem eingelesenen Bild zu findensind, in Form einer Array List aus Rectangle Objekten, zur�ck.
    ...
    public static ArrayList<Rectangle> BildZuRechteck(String path) throws IOException {
        ...
        ArrayList<Rectangle> rechtecke = new ArrayList<>();
        ...
        for(int x = 0; x < schwarz.length; x++){
            for(int y = 0; y < schwarz[0].length; y++){
                if (schwarz[x][y]){
                    ...
                    PositionIO pos = new PositionIO(posX, posY);
    ...
    /**
     * Diese Methode gibt ein PortalIO, welches aus dem Bild erkannt wird zurück.
     * @param path Der Pfad des Bildes
     * @return Ein PortalIO
     * @throws IOException Wenn das angegebene Bild nicht existiert
     */
    public static PortalIO BildZuPortal(String path)throws IOException {
        BufferedImage bim;
        bim = ImageIO.read(new File(path));
        ...
        return new PortalIO(rechtecke.get(0), rechtecke.get(1));
    }
    // Ende Methoden
}
